...
# MiniGridShieldHandler for different visibility frequency

class MiniGridShieldHandler(ShieldHandler):
    def __init__(self, grid_to_prism_binary, grid_file, prism_path, formula, prism_config=None, shield_value=0.9, shield_comparison='absolute', nocleanup=False, prism_file=None, visibility_frequency=0.9) -> None:
        # Initialize attributes as before
        self.tmp_dir_name = f"shielding_files_{datetime.datetime.now().strftime('%Y%m%dT%H%M%S')}_{next(tempfile._get_candidate_names())}"
        os.mkdir(self.tmp_dir_name)
        self.grid_file = self.tmp_dir_name + "/" + grid_file
        self.grid_to_prism_binary = grid_to_prism_binary
        ...
        self.prism_file = prism_file
        self.action_dictionary = None
        self.visibility_frequency = visibility_frequency  # New parameter for visibility frequency
    ...
    def __del__(self):
        # Ensure nocleanup is defined before using it
        if hasattr(self, 'nocleanup') and not self.nocleanup:
    ...
    def create_shield(self, **kwargs):
        # Randomly decide if the shield can view the environment based on visibility frequency; If Value is higher shield is unable to view the environment and taking no action at all.
        
        if random.random() > self.visibility_frequency:
            print("Shield unable to view environment, returning default action mask.")
            return self.default_action_mask()
        
        # Regular shield creation process if shield can view the environment
        if self.action_dictionary is not None:
            return self.action_dictionary
    ...
    def default_action_mask(self):
        # Returns a default mask (or random mask if desired) when shield cannot view the environment
        #action_mask = [1.0] * 7  # Allow all actions by default
        action_mask = [0.0] * 7  # Allow no actions when not visible
        #action_mask = [0, 0 ,0, 0, 0 ,0 ,0 ,1.0]
        return action_mask
...
"""def create_episode_animation(env, shield_handler, episode_steps=100, output_path='episode_animation.gif'):
    # Reset environment and prepare to capture frames
    frames = []
    obs = env.reset()
    ...
    for step in range(episode_steps):
        ...
        # Apply shield overlay if applicable
        shield = shield_handler.create_shield(env=env)
        overlayed_frame = create_shield_overlay_image(env, shield)
        
        # Convert overlayed_frame (PIL Image) to numpy array for consistency
        frame_array = np.array(overlayed_frame)
        
        # Append the frame to our list
        frames.append(Image.fromarray(frame_array))
        
        # Step the environment (random action here for example)
        action = env.action_space.sample()  # Replace with agent's action if available
...
def create_episode_animation(env, shield_handler, episode_steps=100, output_path='episode_animation.gif'):
    ...
    for step in range(episode_steps):
        ...
        # Check if overlayed_frame is already a PIL Image
        if isinstance(overlayed_frame, Image.Image):
            # Directly append PIL Image to frames without conversion
            frames.append(overlayed_frame)
        else:
            # Convert to numpy array if necessary, then to PIL Image
            frame_array = np.array(overlayed_frame)
            frames.append(Image.fromarray(frame_array))
...
def create_shield_overlay_image(env, shield):
    ...
    for x in range(0, env.width):
        for y in range(0, env.height):
            for dir in range(0, 4):
                try:
                    state_key = State(x, y, dir, "")
                    # Ensure the shield contains the state_key and the expected data structure
                    if state_key in shield and isinstance(shield[state_key], list) and len(shield[state_key]) > 2:
                        if shield[state_key][2] <= 0.0:
                            draw.polygon(triangle_for_overlay(x, y, dir, ts), fill=red)
                except KeyError:
                    pass
    img = Image.alpha_composite(img, overlay)
